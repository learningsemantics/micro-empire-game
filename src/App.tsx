...
export default function Home() {
  ...
  const [opsTab, setOpsTab] = useState<"trade" | "team" | "supply" | "market" | "council" | "lab" | "lead" | "life" | "empire" | "people" | "pulse">("trade");
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      if (saved) try {
        ...
        setGame({ ...initialState, ...prior, branches:migratedBranches,residents:prior.residents?.length?prior.residents:makeResidents(),neighbourhoodHeat:{...initialState.neighbourhoodHeat,...(prior.neighbourhoodHeat||{})},skills:{...initialState.skills,...(prior.skills||{})},mentorTrust:{...initialState.mentorTrust,...(prior.mentorTrust||{})},staff:(prior.staff||[]).map((m:StaffMember)=>({...m,tenure:m.tenure||0,loyalty:m.loyalty||70})),segmentSales: { ...initialState.segmentSales, ...(prior.segmentSales || {}) }, customer: null, phase: "home" });
  ...
  function beginNextDay(base?: GameState) {
    ...
    setGame(current => {
      ...
      const next = ev.apply({ ...g, day: nextDayNumber, hour: 9, phase: "play",weather,economy,interestRate,ttcStatus,cityPolicy:policy,cityEvent,neighbourhoodHeat,cityEventLog:[`${nextDayNumber}: ${cityEvent} · ${WEATHER[weather].name} · ${ECONOMY[economy].name}`,...g.cityEventLog].slice(0,8),event: stageUp ? `${STAGES[nextStage].name} unlocked! ${money(grant)} grant, +3 capacity and a skill point.` : `${cityEvent}. ${WEATHER[weather].note}.`,
    ...
    <main className={`game-shell phase-${game.phase}`}>
      ...
      {game.phase === "home" && (
        <section className="home-screen screen">
          <div className="hero-copy">
            ...
            <div className="ribbon">V4.4 · Dynamic Toronto Edition</div>
            <p className="lede">Read a changing city—weather, transit, policy and economic cycles—then build the Toronto empire that can adapt.</p>
      ...
      {game.phase === "play" && business && (
        <section className="play-screen screen">
          ...
          <aside className="action-panel">
            <h3>Founder console</h3><div className="ops-tabs">{(["trade","team","supply","market","council","lab","lead","life","empire","people","pulse"] as const).map(tab => <button key={tab} className={opsTab === tab ? "active" : ""} onClick={() => setOpsTab(tab)}>{tab}</button>)}</div>
            {opsTab === "trade" && <>
      ...
      {game.phase === "result" && (
        <section className="modal-screen"><div className="report-card final-card">
          ...
          <div className="final-validation"><span><small>City responses</small><b>{game.cityActions}</b></span><span><small>Portfolio value</small><b>{money(game.branches.reduce((s,b)=>s+b.propertyValue,0))}</b></span><span><small>Current economy</small><b>{ECONOMY[game.economy].name}</b></span><span><small>Resident loyalty</small><b>{Math.round(game.residents.reduce((s,r)=>s+r.loyalty,0)/game.residents.length)}%</b></span></div>
