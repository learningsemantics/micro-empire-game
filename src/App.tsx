...
type GameState = {
  ...
  weather: WeatherKey; economy: EconomyKey; interestRate: number; ttcStatus: string;
  cityPolicy: PolicyKey; cityEvent: string; cityEventLog: string[];
  neighbourhoodHeat: Record<CityKey,number>; cityActions: number; lastCityActionDay:number;
  rivals: RivalFounder[]; leaguePoints:number; alliances:number; rivalWins:number;
  seasonName:string; lastRivalActionDay:number; leagueCodeImported:boolean;
...
const WEATHER:Record<WeatherKey,{name:string;icon:string,demand:number,travel:number,note:string}>={clear:{name:"Clear",icon:"☀",demand:1.08,travel:0,note:"Strong foot traffic across the city"},rain:{name:"Rain",icon:"☂",demand:.88,travel:2,note:"Indoor demand rises while streets slow"},snow:{name:"Snow",icon:"❄",demand:.72,travel:4,note:"Disruptions suppress traffic and deliveries"},heat:{name:"Heat Alert",icon:"☼",demand:.94,travel:3,note:"Waterfront activity rises; founder energy falls"}};
const ECONOMY:Record<EconomyKey,{name:string;factor:number,note:string}>={growth:{name:"Expansion",factor:1.14,note:"Demand and property values are rising"},steady:{name:"Stable",factor:1,note:"Balanced demand and predictable costs"},slowdown:{name:"Slowdown",factor:.82,note:"Customers scrutinize every purchase"}};
const POLICIES:Record<PolicyKey,{name:string,note:string}>={none:{name:"Status quo",note:"No active citywide intervention"},smallbiz:{name:"Main Street Grant",note:"Permitted businesses can claim growth support"},rentcontrol:{name:"Commercial Rent Relief",note:"Flagship rent growth is temporarily constrained"},green:{name:"Green Toronto Standard",note:"Local suppliers and low-carbon travel receive benefits"}};
const makeRivals=():RivalFounder[]=>[
  {id:"sienna",name:"Sienna Park",company:"North & Pine",business:"coffee",city:"kensington",strategy:"Community brand",cash:1350,reputation:58,locations:1,score:2400,relationship:0,momentum:2},
  {id:"malik",name:"Malik Thompson",company:"Career North",business:"career",city:"financial",strategy:"Premium outcomes",cash:1800,reputation:62,locations:1,score:2750,relationship:0,momentum:3},
  {id:"elena",name:"Elena Rossi",company:"Civic Automations",business:"agency",city:"mars",strategy:"Product innovation",cash:2100,reputation:55,locations:1,score:2900,relationship:0,momentum:4},
...
const initialState: GameState = {
  ...
  weather:"clear",economy:"steady",interestRate:5.25,ttcStatus:"Good service",cityPolicy:"none",cityEvent:"Opening week across Toronto",cityEventLog:[],neighbourhoodHeat:{junction:48,kensington:62,financial:70,harbour:58,liberty:68,yorkville:76,mars:72,cityhall:55},cityActions:0,lastCityActionDay:0,
  rivals:makeRivals(),leaguePoints:0,alliances:0,rivalWins:0,seasonName:"Toronto Founder League · Season One",lastRivalActionDay:0,leagueCodeImported:false,
...
export default function Home() {
  ...
  function respondToCity(kind:"grant"|"activate"|"resilience"){if(game.lastCityActionDay===game.day)return;if(kind==="grant"&&(game.cityPolicy!=="smallbiz"||game.permitLevel<1))return;if(kind!=="grant"&&game.cash<(kind==="activate"?150:120))return;advance(g=>kind==="grant"?{...g,cash:g.cash+650,cityActions:g.cityActions+1,lastCityActionDay:g.day,message:"Your permitted business secured $650 through the Main Street Grant."}:kind==="activate"?{...g,cash:g.cash-150,expenses:g.expenses+150,dailyExpenses:g.dailyExpenses+150,reputation:clamp(g.reputation+8,0,100),socialCapital:clamp(g.socialCapital+5,0,100),cityActions:g.cityActions+1,lastCityActionDay:g.day,message:`A neighbourhood activation turned ${g.cityEvent} into reputation and social capital.`}:{...g,cash:g.cash-120,expenses:g.expenses+120,dailyExpenses:g.dailyExpenses+120,supplier:"local",capacity:g.maxCapacity,stress:clamp(g.stress-5,0,100),cityActions:g.cityActions+1,lastCityActionDay:g.day,message:"A resilience plan stabilized supply and reduced disruption stress."});beep(790)}
  function partnerRival(id:string){if(game.lastRivalActionDay===game.day||game.cash<150)return;const rival=game.rivals.find(r=>r.id===id);if(!rival)return;advance(g=>{const crosses=rival.relationship<35&&rival.relationship+12>=35;return {...g,cash:g.cash-150,expenses:g.expenses+150,dailyExpenses:g.dailyExpenses+150,rivals:g.rivals.map(r=>r.id===id?{...r,relationship:clamp(r.relationship+12,-50,100),score:r.score+80}:r),alliances:g.alliances+(crosses?1:0),network:clamp(g.network+6,0,100),lastRivalActionDay:g.day,message:`A joint activation with ${rival.name} strengthened both companies${crosses?" and formed a formal alliance":""}.`}});beep(750)}
  function challengeRival(id:string){if(game.lastRivalActionDay===game.day)return;const rival=game.rivals.find(r=>r.id===id);if(!rival)return;const win=calculateFounderScore(game)+game.focus*10>=rival.score;advance(g=>({...g,rivals:g.rivals.map(r=>r.id===id?{...r,relationship:clamp(r.relationship-10,-50,100),momentum:clamp(r.momentum+(win?-1:1),1,7)}:r),reputation:clamp(g.reputation+(win?7:-3),0,100),stress:clamp(g.stress+(win?3:9),0,100),leaguePoints:g.leaguePoints+(win?15:0),rivalWins:g.rivalWins+(win?1:0),lastRivalActionDay:g.day,message:win?`You outperformed ${rival.name} in a public founder challenge. +15 league points.`:`${rival.name} won this round. Improve the fundamentals before a rematch.`}));beep(win?900:360)}
  function exportLeague(){const code=btoa(JSON.stringify({v:5,seed:game.seed,difficulty:game.difficulty,season:game.seasonName,heat:game.neighbourhoodHeat,rivals:game.rivals.map(r=>({id:r.id,score:r.score,momentum:r.momentum}))}));navigator.clipboard?.writeText(code);setShareStatus("League challenge code copied");window.setTimeout(()=>setShareStatus(""),1800)}
  function importLeague(){const code=window.prompt("Paste a Toronto Founder League code");if(!code)return;try{const data=JSON.parse(atob(code.trim()));if(data.v!==5)throw new Error();setGame(g=>({...g,seed:Number(data.seed)||g.seed,difficulty:data.difficulty||g.difficulty,seasonName:data.season||"Community League",neighbourhoodHeat:{...g.neighbourhoodHeat,...data.heat},rivals:g.rivals.map(r=>{const shared=data.rivals?.find((x:{id:string})=>x.id===r.id);return shared?{...r,score:Number(shared.score)||r.score,momentum:Number(shared.momentum)||r.momentum}:r}),leagueCodeImported:true,message:"Community league imported. Everyone now plays the same Toronto conditions."}));setShareStatus("Community league imported")}catch{setShareStatus("Invalid V5 league code")}}
    ...
    <main className={`game-shell phase-${game.phase} weather-${game.weather} economy-${game.economy}`}>
      ...
      {game.phase === "play" && business && (
        <section className="play-screen screen">
          ...
          <aside className="action-panel">
            ...
            {opsTab === "pulse" && <div className="ops-list toronto-pulse"><h4>Toronto Pulse</h4><div className="pulse-grid"><span><i>{WEATHER[game.weather].icon}</i><small>Weather</small><b>{WEATHER[game.weather].name}</b></span><span><i>↔</i><small>TTC</small><b>{game.ttcStatus}</b></span><span><i>↗</i><small>Economy</small><b>{ECONOMY[game.economy].name}</b></span><span><i>%</i><small>Interest</small><b>{game.interestRate.toFixed(2)}%</b></span></div><p className="pulse-note">{WEATHER[game.weather].note}. {ECONOMY[game.economy].note}.</p><h4>City policy</h4><article className="policy-card"><strong>{POLICIES[game.cityPolicy].name}</strong><span>{POLICIES[game.cityPolicy].note}</span></article><h4>Major event</h4><article className="event-card"><strong>{game.cityEvent}</strong><span>Local conditions may amplify a matching branch.</span></article><div className="city-response"><button onClick={()=>respondToCity("grant")} disabled={game.lastCityActionDay===game.day||game.cityPolicy!=="smallbiz"||game.permitLevel<1}>Claim grant</button><button onClick={()=>respondToCity("activate")} disabled={game.lastCityActionDay===game.day||game.cash<150}>Activate · $150</button><button onClick={()=>respondToCity("resilience")} disabled={game.lastCityActionDay===game.day||game.cash<120}>Resilience · $120</button></div><h4>Neighbourhood heat</h4>{(Object.keys(CITY) as CityKey[]).map(key=><div className="heat-row" key={key}><span>{CITY[key].name}</span><i><u style={{width:`${game.neighbourhoodHeat[key]}%`}}/></i><b>{game.neighbourhoodHeat[key]}</b></div>)}<h4>City history</h4>{game.cityEventLog.slice(0,4).map((entry,i)=><p className="pulse-log" key={i}>{entry}</p>)}</div>}
            {opsTab === "league" && <div className="ops-list founder-league"><h4>{game.seasonName}</h4><div className="league-hero"><span><small>Your league points</small><b>{game.leaguePoints}</b></span><span><small>Challenge wins</small><b>{game.rivalWins}</b></span><span><small>Alliances</small><b>{game.alliances}</b></span></div><article className="weekly-card"><small>WEEKLY SHARED CHALLENGE</small><strong>{weeklyTheme}</strong><span>{weeklyTheme==="Community Builder"?"Reach 50 social capital":weeklyTheme==="Cash Discipline"?"Finish with $5,000 business cash":weeklyTheme==="Customer Loyalty"?"Build 50% average resident loyalty":"Use local supply and low-carbon transport"}</span><b>{weeklyTheme==="Community Builder"?`${game.socialCapital}/50`:weeklyTheme==="Cash Discipline"?`${money(game.cash)}/$5,000`:weeklyTheme==="Customer Loyalty"?`${Math.round(game.residents.reduce((s,r)=>s+r.loyalty,0)/game.residents.length)}/50`:game.supplier==="local"&&(game.transport==="walk"||game.transport==="bike")?"Complete":"In progress"}</b></article><h4>Live standings</h4>{[{id:"player",name:"You",company:business?.name||"Your Venture",score:calculateFounderScore(game),locations:game.branches.length,reputation:game.reputation,relationship:0},...game.rivals].sort((a,b)=>b.score-a.score).map((r,index)=><article className={`league-row ${r.id==="player"?"player":""}`} key={r.id}><b>{index+1}</b><span><strong>{r.name} · {r.company}</strong><small>{r.locations} locations · Reputation {r.reputation} · Score {r.score.toLocaleString()}</small>{r.id!=="player"&&<small>Relationship {r.relationship}</small>}</span>{r.id!=="player"&&<div><button onClick={()=>partnerRival(r.id)} disabled={game.lastRivalActionDay===game.day||game.cash<150}>Partner</button><button onClick={()=>challengeRival(r.id)} disabled={game.lastRivalActionDay===game.day}>Challenge</button></div>}</article>)}<h4>Community challenge</h4><p className="intel">Share a compact league code so another player receives the same city seed, heat and rival standings.</p><div className="league-share"><button onClick={exportLeague}>Copy league code</button><button onClick={importLeague}>Import league</button></div>{shareStatus&&<p className="share-status">{shareStatus}</p>}</div>}
          </aside>
        </section>
      )}
      ...
      {game.phase === "result" && (
        <section className="modal-screen"><div className="report-card final-card">
          ...
          <div className="final-validation"><span><small>League position</small><b>#{1+game.rivals.filter(r=>r.score>calculateFounderScore(game)).length}</b></span><span><small>League points</small><b>{game.leaguePoints}</b></span><span><small>Rival wins</small><b>{game.rivalWins}</b></span><span><small>Alliances</small><b>{game.alliances}</b></span></div>
